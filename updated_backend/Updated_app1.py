...
# ✅ Correct order: BASE_DIR first, then STATIC_DIR
...
# ------------------------------
# /convert route with metadata injection + PDF STYLING
# ------------------------------
...
def convert_blog_to_pdf():
    try:
        ...
        html = resp.text
        # remove ads first (conservative cleanup)
        try:
            html = remove_ads_from_html(html, source_url=blog_url)
        except Exception as e:
            # If ad removal fails for any reason, continue with original HTML but log
        ...
        # Read custom CSS file for article styling (US-F007)
        custom_css = ""
        if os.path.exists(CSS_FILE):
            with open(CSS_FILE, 'r', encoding='utf-8') as f:
                custom_css = f"<style>{f.read()}</style>"

        # Inject metadata headers/footers AND custom article CSS
        enriched_html = inject_metadata_into_html(html, metadata, css=custom_css)

        # pdfkit/wkhtmltopdf options
        options = {
            ...
            # Margins (header/footer need space)
            'margin-top': '40mm',
            'margin-bottom': '30mm',
            'margin-left': '20mm',
            'margin-right': '20mm',
            # US-F007: Page settings
            'page-size': 'A4',
            'encoding': 'UTF-8',
            'minimum-font-size': 12,
            # US-F007: Image settings
            'image-quality': 95,
            'image-dpi': 300,
            # Enable JavaScript
            ...
            # Custom header for compatibility
        ...
        # Try creating PDF from string; fallback if needed
        try:
            pdfkit.from_string(enriched_html, tmp_pdf_path, configuration=config, options=options)
        except OSError as e:
            # fallback to intermediate HTML file
            print("[US-F003] pdfkit.from_string failed — trying fallback.")
        ...
        print(f"[US-F003] Unexpected error: {str(e)}")
